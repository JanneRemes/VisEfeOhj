...
namespace OpenGLTemplate
{
	...
	public:
		...
		Buffer(const GLuint elementCount, const GLuint elementSize, const BufferType& type, const BufferUsage& usage);
		...
		template<typename T>
		void setData(const std::vector<T>& data, const GLuint elementOffset = 0u);
	...
	private:
		...
		void apply(const GLuint buffer) const;
	...
	template<typename T>
	void Buffer::setData(const std::vector<T>& data, const GLuint elementOffset)
	{
		const GLuint dataSize = sizeof(T) * data.size();
		const GLuint offset = sizeof(T) * elementOffset;

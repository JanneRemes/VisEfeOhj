...
Buffer::Buffer(const GLuint elementCount, const GLuint elementSize, const BufferType& type, const BufferUsage& usage)
	: _buffer(0u),
...
GLuint Buffer::elementCount() const
{
...
{
	apply();
	const GLuint size = _elementSize * _elementCount;
	glBufferData(static_cast<GLenum>(_type), size, nullptr, static_cast<GLenum>(usage));
	OGLT_ASSERT_GL();
	unapply();
}

void Buffer::apply(const GLuint buffer) const
{

...
shared_ptr<Texture> Texture::load(const TextureType& type, const string& filepath)
{
	...
	const GLuint lineByteCount = 4 * width;
...
Texture::Texture(const TextureType& type, const GLuint width, const GLuint height, const vector<GLubyte>& data)
	: _texture(0u),
	  _type(type),
	  _width(width),
	  _height(height)
...
void Texture::apply(const GLuint texture) const
{
...
void Texture::initialiseStorage(const vector<GLubyte>& data)
{
	apply();

...
void BufferState::setVertexAttribute(const GLuint index, const GLfloat value) const
{
	assert(_isInitialised);
	apply();
...
void BufferState::setVertexAttribute(const GLuint index, const vec2& value) const
{
	assert(_isInitialised);
	apply();
...
void BufferState::setVertexAttribute(const GLuint index, const vec3& value) const
{
	assert(_isInitialised);
	apply();
...
void BufferState::setVertexAttribute(const GLuint index, const vec4& value) const
{
	assert(_isInitialised);
	apply();
...
{
	const GLuint vertexSize = calculateVertexSize(vertexFormat);
	GLuint offset = 0u;
...
void BufferState::apply(const GLuint vertexArray) const
{
...
GLuint BufferState::calculateVertexSize(const VertexFormat& vertexFormat)
{
	GLuint size = 0u;
...
GLuint BufferState::getVertexElementTypeSize(const VertexElementType type)
{
	switch(type)
	{
